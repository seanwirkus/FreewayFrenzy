...
static void update_physics(float dt, int steer, int throttle) {
    ...
    /* Speed */
    gs.base_speed = 75.0f + gs.distance * 0.045f;
    ...
    if (boosting) {
        gs.boost_hold_s += dt;
        if (gs.boost_hold_s > 5.0f) gs.boost_hold_s = 5.0f;
    } else {
        gs.boost_hold_s -= dt * 2.0f;
        if (gs.boost_hold_s < 0.0f) gs.boost_hold_s = 0.0f;
    }

    float boost_ramp = gs.boost_hold_s / 5.0f;
    if (boost_ramp > 1.0f) boost_ramp = 1.0f;

    float target_speed;
    if (boosting) {
        float boost_strength = 0.6f * (1.0f + boost_ramp);
    ...
    float sp_lerp = (speed_mod < -0.1f) ? 5.0f : 3.0f;
    ...
    gs.road_scroll += gs.speed * 2.5f * dt;
    if (gs.road_scroll > 100000.0f) gs.road_scroll -= 100000.0f;
    gs.distance += (uint32_t)(gs.speed * dt * 0.3f);
    gs.score = gs.distance / 10;
